...
- [ ] **M13 — P2 — publicação independente por framework.** Hoje `build-push` depende do resultado agregado de `validate`: uma falha de `dotnet8` bloqueia a publicação de todos os frameworks selecionados. Reestruturar as dependências para que cada publicador dependa da validação do seu próprio framework, preservando ambas as arquiteturas e a separação de permissões. Não resolver com `continue-on-error` no scan nem apenas com `if: always()` no publicador. **Aceite:** num lote com um framework reprovado e outro aprovado, só o aprovado publica; o reprovado continua visivelmente falho, sem artifact autorizado/publicação; retry do publicador aprovado reutiliza o mesmo OCI e digest, sem rebuild. Falha numa dependência comum, como o bundle, continua bloqueando todos os dependentes.
