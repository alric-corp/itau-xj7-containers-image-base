...
- [ ] **M14 — P2 — limites de duração e concorrência adequados a cada operação.** Definir `timeout-minutes` nos jobs executores de build, validação, publicação, promoção e testes a partir dos tempos observados. Cancelar validações obsoletas do mesmo PR quando chegar novo commit; separar esses grupos dos jobs autenticados. Preservar a serialização de seleção/promoção por framework, sem cancelar uma promoção em andamento devido a um novo PR/build. Usar retries limitados somente em falhas transitórias de rede, após verificar idempotência; falhas de digest, assinatura ou CVE continuam definitivas. **Aceite:** dois commits no mesmo PR cancelam a validação antiga; PRs diferentes não se cancelam; teste de timeout termina o job; promoção concorrente mantém as garantias de M04. Não presumir ordem FIFO ou execução de todo run pendente apenas por configurar `concurrency`.
