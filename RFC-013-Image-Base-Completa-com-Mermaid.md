...
- [ ] **M16 — P2 — endurecimento dos workflows e revisão efetiva.** Usar `persist-credentials: false` nos checkouts que não precisam fazer push; revisar permissões dos jobs e entradas manuais (framework pertencente ao catálogo e soak numérico não negativo). Verificar secret scanning/push protection, análise de segurança de Actions e CODEOWNERS associado a regras efetivas de revisão para workflows/scripts; registrar o que está habilitado, indisponível ou pendente, sem presumir proteção só pela presença de um arquivo. Evitar checkout de código não confiável em contexto privilegiado e imprimir apenas campos de diagnóstico necessários. **Aceite:** entradas inválidas falham antes de autenticação AWS; checks não precisam de credencial Git persistida; proteção de revisão é demonstrada em PR de teste. O teste de fork continua pertencendo ao M05.
  ...
  - **Aceite comprovado:** proteção de revisão demonstrada no PR real #28 — `reviewDecision: REVIEW_REQUIRED` e `mergeStateStatus: BLOCKED` com os dois required checks `pass`, ou seja, o bloqueio vem da revisão, não de check falho. Checks sem credencial Git persistida: `test` e `lint-workflows` aprovados no [run 34412945439](https://github.com/alric-corp/itau-xj7-containers-image-base/actions/runs/34412945439) já com `persist-credentials: false`. Guard de input coberto por 81 testes de pipeline, incluindo CLI (exit code, catálogo, traversal, digest malformado e ausência de eco do input).
  - **Aceite ainda pendente:** dispatch real com input inválido (o guard só foi exercido localmente e por teste de CLI); PR pós-merge tocando `.github/workflows/**` para demonstrar a exigência de code owner, que hoje ainda não vale porque a `main` carrega o CODEOWNERS inerte; e a tentativa real de merge sem aprovação, que não foi feita para não arriscar mesclar o PR caso a configuração não estivesse ativa. O teste de fork continua pertencendo ao M05.
...
- [ ] **M09 — restante:** versões efetivas por etapa já são registradas nos artifacts e `renovate.json` prepara as propostas de atualização de digest, mas o Renovate não foi ativado e nenhum PR real de atualização existe ainda; os PRs do Dependabot continuam sem revisão. Desde esta entrega, `sha_pinning_required` está ligado no repositório: a plataforma passa a exigir o SHA completo que o repo já usava por convenção.
- [ ] **M16 — restante:** endurecimento de código completo e proteção de revisão ativa e comprovada em PR real (ver item M16 acima). Faltam três evidências: dispatch real com input inválido, PR pós-merge tocando `.github/workflows/**` sob a regra de code owner (a `main` ainda carrega o CODEOWNERS inerte) e a tentativa real de merge sem aprovação. Teste de fork pertence ao M05, já fechado.
