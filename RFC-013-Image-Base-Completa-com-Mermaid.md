...
- [ ] **M12 — P1 — checks rápidos e obrigatórios do pipeline.** O workflow `test-promotion.yml` já executa testes Python e de certificados, mas não roda para toda mudança nos workflows principais e não executa `actionlint`. Ampliar os gatilhos para workflows, scripts e configurações relevantes; automatizar lint e testes antes de permitir publicação. Incluir análise de shell e padrões inseguros de Actions conforme aplicável. Tratar `cve-triage.lock.yml` como arquivo gerado: verificar sua geração a partir da fonte e usar ferramentas compatíveis, sem editar o lock manualmente para satisfazer o lint. **Aceite:** um erro sintático e uma regressão de teste introduzidos deliberadamente bloqueiam o check e a publicação; PR só de documentação recebe resultado definido sem deixar check obrigatório eternamente pendente. Conferir a configuração efetiva dos required checks no GitHub.
