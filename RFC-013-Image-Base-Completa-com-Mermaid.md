...
- [ ] **M04/M06 — integração autenticada:** validar soak/concorrência sob execuções concorrentes reais e compatibilidade com a configuração de imutabilidade dos repositórios de consumo. Essas evidências não são substituídas pelo teste de M03 acima (que rodou com `soak-hours=0`, não testa o agendamento horário real nem concorrência).
...
- [ ] **M04 — validação remota:** confirmar execução horária e ausência de regressão com execuções concorrentes no GitHub/ECR; medir atrasos de fila e scheduler. O teste de M03 usou `soak-hours=0` via dispatch manual, não testa o agendamento real nem concorrência.
