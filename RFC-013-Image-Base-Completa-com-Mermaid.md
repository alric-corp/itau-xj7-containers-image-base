...
- [ ] **M15 — P2 — recuperação de `stable` por digest.** Documentar e testar um runbook para escolher um digest anterior, verificar plataformas, assinatura/provenance e scan segundo a política vigente, registrar motivo/operador e restaurar a referência sem rebuild. Coordenar a recuperação com o mesmo grupo de concorrência da promoção e prever como impedir que o próximo cron repromova imediatamente o candidato retirado. Preservar imagens/evidências pelo período de recuperação definido. **Aceite:** exercício primeiro em repositório isolado confirma o digest de `stable` por leitura de volta e a retomada controlada da promoção. Restaurar a imagem base não reconstrói nem reverte automaticamente aplicações consumidoras; isso deve constar do runbook downstream. Exceções ao gate de CVE exigem política explícita, não bypass implícito.
