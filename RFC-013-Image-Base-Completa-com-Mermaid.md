...
- [ ] **M02 — execução autenticada:** integrar esta entrega na `main` e despachar um framework aprovado (por exemplo, `go1-26`); conferir o artifact `publication-*`, assinatura/provenance e retry parcial do publicador. Scan do lote completo pode bloquear a publicação por `.NET 8`; não reduzir a política de CVEs para contornar o gate.
- [ ] **M03/M04/M06 — integração autenticada:** validar gate de promoção, soak/concorrência e compatibilidade com imutabilidade/assinaturas após integração. Essas evidências não são substituídas pelo teste em registry local.
...
- [ ] **M02 — integração final:** validar no workflow autenticado a cópia OCI já implementada e comprovada manualmente no ECR, incluindo obtenção do artifact do run/tentativa e falhas por adulteração. A sétima entrega prepara a extração dos workflows; falta integrar na `main` e executar o publicador autenticado, inclusive a leitura de volta do índice e dos manifests por arquitetura.
