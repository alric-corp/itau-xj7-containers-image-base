...
- [x] **M07 — aplicações mínimas executadas de verdade:** (1) Go — binário estático compilado com `go1-26-dev`, copiado e executado com sucesso em `go1-26` (runtime), confirmado que `/usr/bin/go` e `/bin/sh` não existem nele. (2) Java — `Main.class` compilado com `javac` em `java21-dev`, executado com sucesso em `java21` (runtime) usando o mesmo `JAVA_HOME=/usr/lib/jvm/java-21-openjdk` do JDK (path confirmado, não só assumido) — e confirmado que `javac` não existe no runtime. (3) .NET — `dotnet --list-runtimes` no runtime confirma `Microsoft.AspNetCore.App` e `Microsoft.NETCore.App` presentes; `dotnet build` falha com `"No .NET SDKs were found"` como esperado (sem SDK). Não foi feito um `dotnet publish` completo (precisa de restore NuGet); a evidência de SDK ausente + runtime presente já comprova a separação.
...
- [ ] **M07 — validação remota:** as 6 novas variantes foram adicionadas às listas de frameworks dos workflows, mas ainda não passaram por um build/scan real no GitHub Actions.
...
- [ ] **M07 — parcial:** Go/.NET/Java separados em runtime + `-dev`, tamanho/inventário comparados e aplicações mínimas executadas de verdade (ver Décima segunda entrega). Falta: `go1-25`/`dotnet8`/`java25` ainda sem separação, e nenhuma das 6 variantes novas passou por build/scan real no GitHub Actions ainda.
...
| 09/09/2026 | Décima segunda entrega: separação build/runtime (M07) para Go, .NET e Java | `openjdk-21-jre`/`aspnet-10-runtime` confirmados reais no APKINDEX do Wolfi; `go1-26.yaml`/`dotnet10.yaml`/`java21.yaml` viraram runtime-only, conteúdo anterior movido para `-dev.yaml`. Redução real: Go 236K vs 51M, .NET 69M vs 264M, Java 72M vs 98M. Binário Go estático e classe Java compiladas nas variantes `-dev` executadas com sucesso nas variantes runtime (JAVA_HOME confirmado, não assumido); `.NET` runtime confirma ASP.NET Core presente e SDK ausente. 59 testes + actionlint aprovados; build multi-arch confirmado. | `go1-25`/`dotnet8`/`java25` sem separação ainda; as 6 variantes novas não passaram por build/scan real no GitHub Actions. |
