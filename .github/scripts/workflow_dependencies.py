"""Resolve the reviewed shared workflows for offline contract/retention checks.

CI checks out the SHA read from the real callers. Local checks use the same
checkout at .reusable-workflows, or REUSABLE_WORKFLOWS_PATH. A missing checkout,
floating ref, divergent pin or altered workflow fails instead of hiding a gate.
"""
import argparse
import os
...
REFERENCE = re.compile(re.escape(REPOSITORY) + r'/(\.github/workflows/[\w-]+\.yml)@([0-9a-f]{40})')
...
def dependencies(root=ROOT):
    ...
    for path in local_workflows(root):
        ...
        for job_id, job in (document.get('jobs') or {}).items():
            ...
            found.append({'caller': path, 'job': job, 'path': match[1], 'sha': match[2]})
    if not found:
        raise ValueError('no shared workflow callers found')
    if len({entry['sha'] for entry in found}) != 1:
...
def shared_workflows(root=ROOT, checkout=None):
    ...
    if not (checkout / '.git').exists():
        raise ValueError('missing shared checkout; set REUSABLE_WORKFLOWS_PATH or check out '
                         f'{REPOSITORY}@{entries[0]["sha"]} into {checkout}')
    head = subprocess.run(['git', '-C', str(checkout), 'rev-parse', 'HEAD'], check=True,
                          capture_output=True, text=True).stdout.strip()
    if head != entries[0]['sha']:
        raise ValueError('shared checkout HEAD differs from the SHA used by callers')
    files = []
...
def main():
    ...
    try:
        if args.mode == 'checkout':
            ...
            from lint_workflow_hardening import check
            problems = []
